...
def _build_findings_by_section(by_section, control_label="CIS ID"):
    if not by_section:
...
<div class="section-block">
  ...
  <table>
    <thead><tr>
      <th>Severity</th><th>{_esc(control_label)}</th><th>Finding</th><th class="center">Resources</th><th class="center">Level</th>
    </tr></thead>
...
def generate_html_report(report_data: dict, output_path: str = "mdc_report.html") -> str:
    ...
    # Resolve the active compliance framework from metadata (defaults to CIS for
    # older JSON snapshots that predate framework selection).
    fw_meta       = meta.get("framework", {})
    fw_name       = fw_meta.get("name", "cis")
    fw_label      = fw_meta.get("label", "CIS Azure Foundations Benchmark v2.0.0")
    control_label = fw_meta.get("control_label", "CIS ID")
    framework     = get_framework(fw_name) if _FRAMEWORK_AVAILABLE else None

    section_heading = {
        "cis":  "Findings by CIS Section",
        "nist": "Findings by NIST CSF Function",
        "none": "Findings",
    }.get(fw_name, "Findings by Section")

    # Backfill expected_value / current_value_if_fail from the active framework
    # mapping for JSON snapshots generated before these fields were added.
        ...
        for section_data in by_section.values():
            for rec in section_data.get("items", []):
                if rec.get("expected_value") is None or rec.get("cis_doc_url") is None:
                    m = framework.lookup_control(rec.get("name", ""))
                    if m:
...
<header>
  ...
  <div class="meta">
    Subscription: {_esc(sub_id)} &nbsp;&bull;&nbsp; Generated: {_esc(display_time)} &nbsp;&bull;&nbsp; Framework: {_esc(fw_label)}
  </div>
</header>
...
<main>
  ...
  {_build_findings_by_section(by_section, control_label)}
...
<footer>
  Generated by mdc_assess.py &bull; {_esc(fw_label)}
</footer>
...
def generate_csv_report(report_data: dict, output_path: str = "mdc_report.csv") -> str:
    ...
    fw_meta = report_data.get("report_metadata", {}).get("framework", {})
    fw_label = fw_meta.get("label", "CIS Azure Foundations Benchmark v2.0.0")
    control_label = fw_meta.get("control_label", "CIS ID")
    control_col = f"{control_label} ({fw_label})"
    ...
    for section_name, data in by_section.items():
        ...
        for item in data.get("items", []):
            ...
            rows.append({
                ...
                control_col:          item.get("cis_id") or "N/A — not an Azure control",
                "Section":            cis_section,
